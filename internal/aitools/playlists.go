...
func playlistTools() []Tool {
	return []Tool{
		...
		},
		{
			...
			Description: "Add up to 100 items to a playlist by their full Spotify URIs (e.g. spotify:track:4iV5W9uYEdYUVa79Axb7Rh). Appends by default; pass 'position' to insert at a zero-based index. Returns the playlist's new snapshot ID.",
			Schema: schema(map[string]any{
				"id":       str("The Spotify ID of the playlist."),
			...
			Handler: func(ctx context.Context, c *spotify.Client, input json.RawMessage) (any, error) {
				...
				id, err := c.AddItemsToPlaylist(ctx, args.ID, args.URIs, opts...)
				if err != nil {
					return nil, err
				}
